...
def clean_title(name):
    for word in name.split():
        if word.endswith('.'):
            title = word
            if title == 'Mr.': title = -1
            elif title == 'Miss.': title = -2
            elif title == 'Mrs.': title = -3
            elif title == 'Master.': title = -4
            elif title == 'Dr.': title = -5
            else: title = -6
    return title

def clean_cabin(cabin):
    if cabin == 'B' or 'D' or 'E': cabin = 1 #80% survival rate
    elif cabin == 'C' or 'F': cabin = 2 #70% survival rate
    elif cabin == '': cabin = 4 #empty cabin
